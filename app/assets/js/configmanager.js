...
const userAppDataDir = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Preferences' : '/var/local'); // Diretório de dados do usuário (AppData no Windows, ~/.config no Linux)
...
const dataPath = path.join(sysRoot, '.pxbrlauncher');
...
function defaultJavaConfig() {
    const javaExecutablePath = path.join(pxbrlauncherDir, 'jre', 'bin', 'javaw.exe');
...
/**
 * Adds a cracked account to the database to be stored.
 * 
 * @param {string} username The username of the cracked account.
 * @returns {Object} The cracked account object created by this action.
 */
exports.addAccount = function(username) {
    ...
    config.authenticationDatabase[username] = accountObj
    return accountObj
}
